...
void PEDIT01::fill_hilight_shadow()
{
	...
	for ( i = 0 ; i < ZAREAD ; i++ )
	{
		...
		if ( !data.at( dl )->isValidFile() || data.at( dl )->il_excl )  { continue ; }
...
void PEDIT01::actionPrimCommand1()
{
	...
	case PC_RESET:
			...
			else if ( w2 == "CMD")
			{
				...
				else if ( !getLabelItr( w3, its, i ) || !getLabelItr( w4, ite, j ) ) { MSG = "PEDT011F" ; return ; }
				else if ( i > j )
				{
					it  = its ;
					its = ite ;
...
bool PEDIT01::checkLineCommands()
{
	...
	for ( it = data.begin() ; it != data.end() ; it++ )
	{
		...
		if ( (*it)->il_lcc[ 0 ] == '.' )
		{
			...
			if ( !(*it)->il_file )              { MSG = "PEDT011E" ; return false ; }
...
void PEDIT01::createFile( uint sidx, uint eidx )
{
	...
	for ( i = sidx ; i <= eidx ; i++ )
	{
		...
		if ( profXTabs )
		{
			...
			for ( j = 0 ; j < t1.size() ; j++ )
			{
				if ( (j % 8 == 0) && t1.size() > j+7 && t1.compare( j, 8, "        " ) == 0 )
				{
					t2.push_back( '\t' ) ;
					j = j + 7 ;
				}
