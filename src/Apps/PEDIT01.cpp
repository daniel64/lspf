...
/*   8/4  Cannot use edit on file.  Browse instead (record length > 65,535)                  */
...
void PEDIT01::parse_parms()
{
	...
	edimac = parseString( err, PARM, "MACRO()" ) ;
	if ( err.error() )
	{
		uabend( "PEDT015", PARM ) ;
	}

	edprof = parseString( err, PARM, "PROFILE()" ) ;
	if ( err.error() )
	{
		uabend( "PEDT015", PARM ) ;
	}

	edlmac = parseString( err, PARM, "LINECMDS()" ) ;
	if ( err.error() )
	{
		uabend( "PEDT015", PARM ) ;
	}

	t = parseString( err, PARM, "CONFIRM()" ) ;
	if ( err.error() || ( t != "" && t != "NO" && t != "YES" ) )
	{
		uabend( "PEDT015", PARM ) ;
	}
	optConfCancel = ( t == "YES" ) ;
...
void PEDIT01::Edit()
{
	...
	if ( t != "NONE" )
	{
		...
		if ( !profLock && t != "CANCEL" ) { profIMAC = t ; }
	}
...
void PEDIT01::addEditRecovery()
{
	...
	// zedtfile - file being edited (zfile)
	// zedbfile - backup destination (bfile - set during error recovery)
	...
	while ( true )
	{
		...
		if ( zedstat == "0" )
		{
			...
			zedbfile = bfile ;
			zedmode  = "E"   ;
			zedopts  = optConfCancel ? "1" : "0" ;
			zedopts  = zedopts + ( optPreserve ? "1" : "0" ) ;
			zeduser  = ""    ;
			tbput( tabName ) ;
			if ( RC != 0 )
			{
...
void PEDIT01::readFile()
{
	...
	// bfile - edit recovery destination file (original file name being edited)
	...
	fname = zfile ;
	vget( "ZEDTFILE", SHARED ) ;
	vcopy( "ZEDTFILE", tfile, MOVE ) ;
	vget( "ZEDBFILE", SHARED ) ;
	vcopy( "ZEDBFILE", bfile, MOVE ) ;
	if ( bfile == zfile )
	{
		zfile         = tfile ;
		fileChanged   = true ;
		abendRecovery = true ;
		vreplace( "ZEDTFILE", "" ) ;
		vput( "ZEDTFILE", SHARED ) ;
		vreplace( "ZEDBFILE", "" ) ;
		vput( "ZEDBFILE", SHARED ) ;
	}
	else
	{
		fileChanged = false ;
...
void PEDIT01::actionChange()
{
	...
	else if ( fcx_parms.f_regreq )
	{
		...
		auto ite = temp.end() ;
		regex_replace( back_inserter( temp1 ), its, ite, regexp, fcx_parms.f_cstring, boost::format_first_only ) ;
...
int PEDIT01::getNextValidURID( vector<iline*>::iterator it )
{
...
void PEDIT01::addSpecial( LN_TYPE t, vector<iline*>::iterator it, const string& s )
{
	iline* p_iline ;
...
void PEDIT01::readLineCommandTable()
{
	...
	while ( RC == 0 )
	{
		if ( datatype( zelcnam, 'U' ) && t2lineCmds.count( zelcnam ) == 0 )
		{
			t2lineCmds[ zelcnam ] = LC_LMAC ;
			t.lcname  = zelcnam ;
			t.lcmac   = zelcmac ;
			t.lcpgm   = ( zelcpgm == "Y" ) ;
			lmacs[ zelcnam ] = t ;
			t2blkCmds.insert( blkcmd ) ;
			if ( zelcmlt == "Y" ) { t2reptOk.insert( zelcnam )  ; }
			if ( zelcdst == "Y" ) { t2abokReq.insert( zelcnam ) ; }
			if ( zelcblk == "Y" && zelcnam.size() < 6 )
			{
				blkcmd = zelcnam + zelcnam.back() ;
				t2lineCmds[ blkcmd ] = LC_LMAC ;
				t2blkCmds.insert( blkcmd ) ;
				if ( zelcdst == "Y" ) { t2abokReq.insert( blkcmd ) ; }
				lmacs[ blkcmd ] = t ;
			}
		}
		tbskip( edlmac ) ;
