	...
	for ( dl = topLine ; dl < data.size() ; dl++ )
	{
		...
		if ( (*it)->il_file )
		{
			if ( (*it)->il_hex || profHex )
			{
				...
				t2 = cs2xs( t1 ) ;
				ln = (*it)->get_idata().size() - startCol + 1 ;
				ln = t1.size() ;
				if ( ln > 0 )
				{
					i = 0 ;
...
void PEDIT01::actionZVERB()
{
	...
	else if ( ZVERB == "RCHANGE" )
	{
		...
		else
		{
			...
			if ( find_parms.fcx_dir     == 'N' ||
			     find_parms.fcx_dir     == 'F' ) { MSG = "PEDT013V"             ; }
			else                                 { MSG = "PEDT013W"             ; }
		}
...
bool PEDIT01::setFindChangeExcl( char type )
{
	...
	if ( t.fcx_string == "" ||
	   ( type == 'C' && t.fcx_cstring == "" && !f_str2 )) { MSG = "PEDT011D" ; return false ; }
